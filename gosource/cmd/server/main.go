...
func (s *server) artworkPage(w http.ResponseWriter, r *http.Request) {
	...
	svgCode := ``
	if aw.SVGData.Valid {
		pngRenderURL := s.baseURL + "/api/og/" + urlPathEscape(id) + ".png"
		svgCode = `<div class="mt-4"><a class="text-sm text-purple-400 hover:text-purple-300 transition-colors" href="` + pngRenderURL + `">View PNG rendering</a></div><details class="mt-3 bg-gallery-card rounded-xl border border-gallery-border"><summary class="p-4 cursor-pointer text-sm text-zinc-400 hover:text-white transition-colors">View SVG Code</summary><pre class="p-4 pt-0 text-xs text-zinc-400 overflow-x-auto font-mono">` + template.HTMLEscapeString(aw.SVGData.String) + `</pre></details>`
	}
